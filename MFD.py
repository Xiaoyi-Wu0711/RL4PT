...
class Travelers():
    ...
    def sell_and_buy(self, _t, _currToll, _toll, _price, _totTime, toll):
        ...
        if self.ARway == 'lumpsum':
            ...
            # ### if trade happens, check their buy and sell behaviors
            # if int(np.count_nonzero(userBuy))!= 0:
            #     path1 ="userAccounts_buy_"+str(_t)+".npy"
            #     np.save(path1, self.userAccounts)
            #     path2 ="departureTime_buy_"+str(_t)+".npy"
            #     np.save(path2, departureTime)            
            #     path3 ="toll_buy_"+str(_t)+".npy"
            #     np.save(path3, toll)     
            #     # path4 ="toll_para_buy_"+str(_t)+".npy"
            #     # np.save(path4, action)       

            # if int(np.count_nonzero(userSell))!= 0:
            #     path1 ="userAccounts_sell_"+str(_t)+".npy"
            #     np.save(path1, self.userAccounts)
            #     path2 ="departureTime_sell_"+str(_t)+".npy"
            #     np.save(path2, departureTime)
            #     path3 ="toll_sell_"+str(_t)+".npy"
            #     np.save(path3, toll)
            #     # path4 ="toll_para_sell_"+str(_t)+".npy"
            #     # np.save(path4, _currToll)  
