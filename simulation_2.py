...
class CommuteEnv(Env): # env reset for each training/testing
    ...
    def reset(self): # env reset for each episode
    ...
    def step(self, action):
        ...
        if 2*(self.toll_A+action[1]) + 3 < 1:
            self.toll_A = -1
        elif 2*(self.toll_A+action[1]) + 3 > 5:
            self.toll_A = 1
        else:
            self.toll_A += action[1]
