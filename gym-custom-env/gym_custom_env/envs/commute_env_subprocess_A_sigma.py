...
class SubProcess_CommuteEnv_A_sigma(gym.Env): # env reset for each training/testing
    ...
    def reset(self, seed = None, options =None): # env reset for each episode
        ...
        self.toll_sigma = 10*(2*random.random()-1) + 60 
    ...
    def step(self, action):
        ...
        if self.toll_sigma + self.action_weights[1] * action[1] < 50:
            self.toll_sigma = 50
        elif self.toll_sigma + self.action_weights[1] * action[1]> 70:
            self.toll_sigma = 70
        else:
            self.toll_sigma += self.action_weights[1] * action[1]
        ...
        if (self.mode == "simulation"):
            self.sim.toll_parameter = np.array([action[0], self.toll_mu, action[1]])
            self.sim.users.toll_parameter =np.array([action[0], self.toll_mu, action[1]])
            self.sim.toll = np.repeat(np.maximum(self.sim.bimodal(timeofday[np.arange(0,self.sim.hoursInA_Day*60,self.sim.Tstep)]),0),self.sim.Tstep)
            self.sim.toll = np.around(self.sim.toll, 2)
