...
class SubProcess_CommuteEnv_A_mu_sigma(gym.Env): # env reset for each training/testing
    ...
    def step(self, action):
        ...
        if self.toll_mu + self.action_weights[1] * action[1] < 300:
            self.toll_mu = 300
        elif self.toll_mu + self.action_weights[1] * action[1]> 540:
            self.toll_mu = 540
        else:
            self.toll_mu += self.action_weights[1] * action[1]

        if self.toll_sigma + self.action_weights[2] * action[2] < 50:
            self.toll_sigma = 50
        elif self.toll_sigma + self.action_weights[2] * action[2]> 70:
            self.toll_sigma = 70
        else:
            self.toll_sigma += self.action_weights[2] * action[2]                
        ...
        if (self.mode == "simulation"):
            self.sim.toll_parameter = np.array([action[0], action[1], action[2]])
            self.sim.users.toll_parameter =np.array([action[0], action[1], action[2]])
            self.sim.toll = np.repeat(np.maximum(self.sim.bimodal(timeofday[np.arange(0,self.sim.hoursInA_Day*60,self.sim.Tstep)]),0),self.sim.Tstep)
            self.sim.toll = np.around(self.sim.toll, 2)
