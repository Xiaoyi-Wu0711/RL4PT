...
class CommuteEnv_reward_shifting(gym.Env): # env reset for each training/testing
        # Define initial parameters for the environment
    def __init__(self, simulation_day_num = 5, save_episode_freq = 9, train=True, save_dir = "./train_result/", space_shape=(4, int(12*60/5)) ):
        super().__init__()
    ...
    def step(self, action):
        ...
        if self.episode == 0:
            mean_value = 34
            reward = mean_value - AITT
        else:
            mean_value = np.mean(self.get_tt_last_5_day())
            reward = mean_value - AITT
