...
class SubProcess_CommuteEnv_sigma(gym.Env): # env reset for each training/testing
    ...
    def reset(self, seed = None, options =None): # env reset for each episode
        ...
        self.toll_sigma = (2*random.random()-1)*10 + 60
    ...
    def step(self, action):
        ...
        if self.toll_sigma + self.action_weights[0] * action[0] < 50:
            self.toll_sigma = 50
        elif self.toll_sigma + self.action_weights[0] * action[0]> 70:
            self.toll_sigma = 70
        else:
            self.toll_sigma += self.action_weights[0] * action[0]
        ...
        if (self.mode == "simulation"):
            self.sim.toll_parameter = np.array([self.toll_A, self.toll_mu, action[0]])
            self.sim.users.toll_parameter =np.array([self.toll_A, self.toll_mu, action[0]])
            self.sim.toll = np.repeat(np.maximum(self.sim.bimodal(timeofday[np.arange(0,self.sim.hoursInA_Day*60,self.sim.Tstep)]),0),self.sim.Tstep)
            self.sim.toll = np.around(self.sim.toll, 2)
        ...
        # print("  np.mean(sigma_tt_interval) ",  np.mean(sigma_tt_interval))
        # print("  self.std_weights * np.mean(sigma_tt_interval) ",  self.std_weights * np.mean(sigma_tt_interval))
        # print(" - AITT_daily/self.sim.fftt ", - AITT_daily/self.sim.fftt)
        # print(" reward: ", reward)
        # print(" ")
