...
class SubProcess_CommuteEnv_A(gym.Env): # env reset for each training/testing
    ...
    def step(self, action):
        ...
        if self.toll_A + self.action_weights[0] * action[0] < 0:
            self.toll_A = 0
        elif self.toll_A + self.action_weights[0] * action[0]> 7:
            self.toll_A = 7
        else:
            self.toll_A += self.action_weights[0] * action[0]
        ...
        if (self.mode == "simulation"):
            self.sim.toll_parameter = np.array([action[0], self.toll_mu, self.toll_sigma])
            self.sim.users.toll_parameter = np.array([action[0], self.toll_mu, self.toll_sigma])
            self.sim.toll = np.repeat(np.maximum(self.sim.bimodal(timeofday[np.arange(0,self.sim.hoursInA_Day*60,self.sim.Tstep)]),0),self.sim.Tstep)
            self.sim.toll = np.around(self.sim.toll, 2)
        ...
        if self.reward_scheme == "Shifting":
        ...
        elif self.reward_scheme == "triangle_add_constant":
            ...
            reward = (-AITT_daily/self.sim.fftt + self.pt_weights * pt_reward + 5)/self.reward_weight

        elif self.reward_scheme == "fftt_plus_pt":
            reward = (- AITT_daily/self.sim.fftt + self.pt_weights *  pt_share_number/self.numOfusers)/self.reward_weight
            ...
            reward = (-AITT_daily/self.sim.fftt + self.pt_weights * pt_reward)/self.reward_weight
